...
to setup
  ...
  set unhappy-turtles turtles with [unhappy?]
...
to go
  ...
  ifelse (not any? unhappy-turtles) or none-moved? [
    set none-moved? false
    stop
  ]
  [
    ...
    ask turtles [
      ifelse unhappy? [
        ...
        if length acceptable-sites > 0 [ 
          ...
          move-to first acceptable-sites
          let p patch-here
...
to-report proportion-like-me [p t]
  ...
  let id [who] of t
  let result 0
  let n-turtles (turtles-on [neighbors] of p) with [who != id]
  if count n-turtles > 0 [ 
    set result (count n-turtles with [color = c]) / count n-turtles
  ]
  report result
end
...
choice-mode
choice-mode
"best-available" "better-than-current" "better-than-required"
1
