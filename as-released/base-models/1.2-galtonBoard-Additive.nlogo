...
globals [
  column-heights ;; record of the height of each column
]
...
to go
  drop-one
  if any? turtles and (max column-heights >= max-pycor) [
    stop
  ]
  tick
end  
...
to drop
 ;; additive -> to build a normal distribution
  while [ycor > 0] [
    let pxcor-new pxcor + one-of [1 -1]
    ;; this may bias the distribution by cutting it off at extreme values 
    if pxcor-new > max-pxcor [die]
    setxy pxcor-new (pycor - 1)
  ]
end    
          
to move-to-final-position
  ;; update the tally for this column and colour the patches appropriately
  let height item (floor xcor) column-heights
  set column-heights replace-item (floor xcor) column-heights (height + 1)
  move-to patch xcor (height + 1)
  set shape "square"
  set size 2
  set color white
end
...
This model is an implementation of a Galton board as discussed in Chapter 1 of
...
Turtles are used to show the heights of histogram bars as the final distribution develops.  This means that at the end of their 'drop' they change colour and are moved to a position that reflects the height of the column they are in.  This is to ensure that histogram bars are drawn on top of the underlying ball drop paths.  If instead patches were used to show histogram bars, the paths would obscure the histogram bars.
