...
patches-own [
  x
  x-start ;; initial x value for reset function
  x-nhb
  centre?
  block-set
...
;; random class in each patch
to rnd-patch-state
  ask patches [
    set x-start random n-types
    set pcolor item x-start color-list 
    set x x-start  
  ]
end   
...
to coarsen
  ...
  ;; find the centre of the neighbourhoods and store in c-list
  let step floor (new-grain / 2) 
  let c-list []
  set c-list lput floor (new-grain / 2) c-list
  
  let L max-pxcor - (new-grain - 1)
  repeat L - 2 [
    set c-list lput (last c-list + new-grain) c-list
  ]
  ;; build the nhbs around each centre and apply modal filter
  ask patches with [member? pxcor c-list and member? pycor c-list ] [
    build-blocks self
    set centre? true
    let mode-x one-of modes [x-start] of block-set
    ask block-set [set x-nhb mode-x]
  ]   
  ;; update the patch values
  ask patches
  [
    set pcolor item x-nhb color-list 
    set x x-nhb
  ]
...
;; build the nhbs around patches (recursive neighbors)
to build-blocks [focal-patch]
  let loops (new-grain - 1) / 2 
