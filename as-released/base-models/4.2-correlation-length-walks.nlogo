...
;__includes ["4.2-r-snapshots-2.nls"]
;
;extensions [r]
...
to setup
  ...
  ; reorient each location towards the next one
  foreach but-last sort locations [
    ask ? [
      set heading [heading] of location (who + 1)
    ]
  ]
end
...
to go
  ...
  ask last-stop [
    ...
    let trn-angle 500 ;; dummy value to force at least one random selection
    while [trn-angle >= 180 or trn-angle < -180] [ 
      set trn-angle random-normal 0 stdev-angle
    ]
    set turn-angles lput trn-angle turn-angles
    set heading heading + trn-angle
    ;; make a new location at the current coordinates
    ;; the new location will move to the next walk location
...
The `go` procedure `hatches` a copy of the most recently created location which is retrieved using `one-of locations with-max [who].  The newly hatched `location` then changes its heading and moves forward a step.
...
The following figures were made using this model, with the following parameter settings:

### Figure 5.6
Make a random walk with `st-dev-angle` set to 30 degrees, and then aggregate it by 15 steps.  You should see a similar transformation in the turn angle distribution to that shown in figure 5.6.
