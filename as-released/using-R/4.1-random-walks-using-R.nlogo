...
turtles-own [
  ...
  real_y
  history-x  ; stores history of xcor of the turtles
  history-y  ; stores history of ycor
]
...
to update-status-monitors
  update-stats
  ; only update plots every 10 ticks
end
...
An alternative version of this model is available that does not require the R-netlogo extension.
...
**normally distributed step length** is the same as the simple case, except that the step length is chosen from a normal distribution.  Note that in this case, the `mean-step-length` parameter is interpretted as the standard-deviation of the normal distribution from which step lengths are drawn, 0 is the mean, and the absolute value of the normally distributed number is used for the step length.
...
**Cauchy distributed step length** is the same as the simple case, except that the step length is chosen from a Cauchy distribution, using the NetLogo R extension, which must be installed.
...
Commercial licenses are also available. To inquire about commercial licenses, please contact David O'Sullivan at d.osullivan@auckland.ac.nz, or George Perry at george.perry@auckland.ac.nz## CREDITS AND REFERENCES
@#$#@#$#@
default
true
0
