...
to setup 
  ...
  ask patches [
    set my-energy 0.5 * (- spin) * sum [spin] of neighbors4 
  ]
  update-display
...
to go 
  ask one-of patches [
    let d-energy spin * sum [spin] of neighbors4
    if random-float 1 < exp ((- d-energy) / T) [
      set spin (- spin)
      set my-energy 0.5 * (- spin) * sum [spin] of neighbors4
      set flips flips + 1 
    ]
...
to update-display
  set energy sum [my-energy] of patches
  ask patches [
    ifelse spin = 1 
    [ set pcolor red ]
    [ set pcolor blue ]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if ticks > 0 [plot flips / ticks]"
