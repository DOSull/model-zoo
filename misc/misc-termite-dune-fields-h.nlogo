...
;  ask one-of (elevated) [ ;; this promotes mobile dunes that _advance_ not retreat...
    ...
    sprout 1 [
      ...
      let v 0
      while [v <= 0] [
        set v random-normal mean-speed sd-speed
      ]
      while [v > 0] [
        if movement-bias? [ set heading 90 ]
        rt random-normal 0 sd-angle
        jump 1
        set v  v - h - friction
      ]
      downhill4 h ;; this important for advance of dunes
      if h = 0 [ set elevated (patch-set patch-here elevated) ]
      set h h + 1
