...
to-report get-n-states 
  report length remove-duplicates [state] of patches
end

to go
  if ticks mod count patches = 0 [
    if length remove-duplicates [state] of patches = 1 [
      stop
    ]
    if not colour-updating? [
      if get-n-states <= 140 [
        let colour-map shuffle sort n-of get-n-states n-values 140 [?] 
        let states remove-duplicates [state] of patches
        let clusters map [patches with [state = ?]] states
        (foreach clusters colour-map [
          ask ?1 [
            set pcolor ?2
            set state ?2
          ]
        ])
        set colour-updating? true
      ]
    ]
  ]
  ask one-of patches [
...
to r-plot-world
  let z map [[pcolor] of ?] (sort patches)
  r:put "z" z
  let n-states length remove-duplicates [pcolor] of patches
  r:put "nr" world-height
  r:put "nc" world-width
  r:put "n_states" get-n-states
  r:eval("greys <- grey(seq(n_states,0,-1)/n_states)")
  r:eval("map <- matrix(z, ncol=nc, nrow=nr)")
  r:eval("image(map, col=greys, asp=1, axes=F)")
end 
...
to createMatrix
  ...
  r:eval "map <- matrix(patches$pcolor, xdim)"
  ; transpose matrix 
  r:eval "map <- t(map)"
end
@#$#@#$#@
