...
to setup
  ...
  repeat number [ make-point ]
  ask patches [ assign points ]
  ask points [ triangulate ]
  ;;color-boundaries patches
...
to make-point
  create-points 1 [
    set size 2
    set color one-of available-colors ;; available-colors
...
to assign [pts] ;; patch procedure
  set parent min-one-of pts [distance myself]
  set pcolor [color] of parent
end

to triangulate
  ask my-links [ die ]
  let n-patches patch-set [neighbors4] of my-poly
  let n-polys turtle-set [parent] of n-patches
  ask other n-polys [
...
;; turtle (point) procedure
;; reports those patches which have self as parent
to-report my-poly
  report patches with [parent = myself]
end
...
to go
  ...
  ifelse mouse-down? [
    ...
    if x != [xcor] of current-point or y != [ycor] of current-point [
      ...
      ask current-point [ 
        setxy x y
        let n-2 turtle-set [link-neighbors] of link-neighbors
        let p-2 patch-set [my-poly] of n-2
        ask p-2 [ assign n-2 ]
        ;; color-boundaries p-2
        ask n-2 [ triangulate ]
      ]
    ]
  ]
...
to-report parents-in-neighborhood [as-list?]
  let x [parent] of neighbors4
  if not member? parent x [set x fput parent x]
  ifelse as-list?
  [ report x ] 
  [ report turtle-set x ] 
end

to-report distinct-parents-in-neighborhood
  report sort remove-duplicates parents-in-neighborhood true
end

to-report assigned-in-neighborhood
  report length filter [? != nobody] parents-in-neighborhood true
end

to color-boundaries [p]
  ask p with [length distinct-parents-in-neighborhood > 1] [
    set pcolor black
  ]
end
...
Commercial licenses are also available. To inquire about commercial licenses, please contact David O'Sullivan at d.osullivan@auckland.ac.nz, or George Perry at george.perry@auckland.ac.nzIn other publications, please use:  Copyright 2006 Uri Wilensky.  All rights reserved.  See http://ccl.northwestern.edu/netlogo/models/Voronoi for terms of use.
@#$#@#$#@
default
true
0
