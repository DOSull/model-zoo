...
to mark-habitat-patches
  let patch-count 1
  let patches-to-mark patches with [to-mark?]
  while [any? patches-to-mark] [
    ...
    while [any? this-cluster] [ 
      ...
      let next-this-cluster (patch-set [neighbors4] of this-cluster) with [to-mark?]
      set this-cluster next-this-cluster
    ]
    ;; increment the patch-count and reset the set to mark
    set patch-count patch-count + 1
    set patches-to-mark patches with [to-mark?]
  ]
  ;; make and store a list of each cluster as a patch-set
  set clusters map [ patches with [class = ?] ] n-values (patch-count) [ ? ]
end
...
to lattice-to-R
  ...
  r:put "n.types" num-classes
  r:eval("grey.ramp <- grey(0:(n.types-1) / n.types)")
    
  r:put "z" map [[class] of ?] reverse sort patches
