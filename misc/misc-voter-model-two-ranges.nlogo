...
to setup 
  ...
  set colours [white black] 
  set queue sort patches
  ask patches [
    set pcolor one-of colours
    set activators patches in-radius activation-range
    set inhibitors patches in-radius inhibition-range with [not (member? self [activators] of myself)]
  ]
  reset-ticks
end
...
to go
  ifelse sequential-update? [
    ask first queue [ update-state ]
    set queue (sentence but-first queue first queue)
  ]
  [ 
...
to update-state
  ifelse random 2 = 1 [
    if [pcolor] of one-of activators = black [
      set pcolor black
    ]
  ]
  [
    if random-float 1 < inhibition-rate [
      if [pcolor] of one-of inhibitors = black [
        set pcolor white
      ]
    ]
  ]
...
to-report get-value
  report position pcolor colours
end
...
to r-plot-world
  ...
  r:put "n_states" max [state] of patches + 1
